...
When ceating/cloning a new git repository example hook files are copied from a system wide directory into every repositories .git/hooks direcotry. CommitHookr's global option uses this to create a global CommitHookr template file which then gets copied into every new repository.
